...
If you are using a library that dynamically injects css into the page (e.g. `styled-components`, `emotion`, or `MUI`), you will need to set your `style-elem-src` policy to `unsafe-inline` **OR** switch to use a `server` & `nonce`.
...
<Callout>
    There are risks to using `unsafe-inline` for your styles. But most of these can be mitigated by a sound CSP. [Read more about this here](https://scotthelme.co.uk/can-you-get-pwned-with-css/)
</Callout>
