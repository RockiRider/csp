...
<Tabs items={['hash']} defaultIndex="0">
  <Tabs.Tab>
    ...
    <Callout type="info">
      The reason this is set to false by default is because you might not neccessarily need to use hashes in production because of the [default policy](/guides/policy#default-policies) including 'self'. This depends on your deployment strategy.
     </Callout>
  </Tabs.Tab>
</Tabs>