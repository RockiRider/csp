...
<Tabs items={['hash']} defaultIndex="0">
  <Tabs.Tab>
    ...
    <Callout type="info">
      The reason this is set to true by default becausing hashing is good for security purposes. If you opt out we still include 'self' in the [default policy](/guides/policy#default-policies). 
     </Callout>
  </Tabs.Tab>
</Tabs>